  ...
  if(req.method === 'POST'){
    req.on('data', chunk => {
      ...
      if(store[k]){
        ...
        let historyLength = history[k].length
        if(historyLength >= 100)
          history[k].splice(0,1)

        let lastEntry = history[k][historyLength]
        if(!lastEntry || JSON.stringify(lastEntry.value) !== JSON.stringify(v)){
          history[k].push({
            meta: meta[k],
            value: store[k],
            archived: Date.now()
